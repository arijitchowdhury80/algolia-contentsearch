...
### ✅ TRANSPARENCY (~2:40am, commit 09e68a1 — pushed + VPS redeployed + live-verified)
- **"Where was the unsupported claim?"** → judge `groundingViolations` were dropped by liveJudge; now threaded → `verdict.violations` → rail "⚠ Flagged as unsupported" card showing the exact claim + reason + Skeptic confidence, with a note that live judging scores vs THIN source snippets (so a sound answer can be flagged → explains the 3.0; batch judge w/ full sources is authoritative). Live-verified: SSE smoke returned 3 violations.
- **"What was the tuning/hardening?"** → config diff was vanity labels; now shows the REAL specifics (tuned-index settings + 7 synonym groups from `optimize_index.mjs`; hardened-prompt rules + bare-concept reformulation from `instructions_case3_reformulation_fix_v2.md`), source files named for verification.
- server 34 + web 88 tests; tsc clean; proof-9. VPS @ 09e68a1, SSE+violations verified on `judge.contentengagement.info`.
- **The 3.0 ≠ tuning failed** — it's the live judge thin-source gate (now visible). Real scorecard = batch `cli judge` (next).
