# Case 3 — v0 BASELINE (frozen 2026-06-12) — rollback anchor

> ⚠️ This is the EXACT Case-3 instruction config the live tuned agent ran at baseline time, frozen before the two-way-discovery deepening was applied to `instructions_case3_optimized_v0.md`. Purpose: (1) preserve the true baseline (no git in this repo), (2) re-deployable rollback anchor for the Autocorrect loop. The baseline judge run must reflect THIS config; the deepened two-way section in the working v0 file is a measured mutation deployed AFTER baseline. Deploy: `node scripts/setup/agent_admin.mjs update b5c4de23-769b-4b38-9051-a19add9dee06 scripts/setup/instructions_case3_v0_baseline.md`
